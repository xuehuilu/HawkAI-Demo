...
interface ConnectRepoModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConnect: (data: { url: string; token: string; branch: string }) => void;
}

export const ConnectRepoModal: React.FC<ConnectRepoModalProps> = ({ isOpen, onClose, onConnect }) => {
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-200 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-slate-800">连接新代码库</h3>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button className="flex items-center justify-center gap-2 p-2 border-2 border-indigo-500 bg-indigo-50 rounded-lg">
                                <span>📦</span>
                                <span className="font-semibold text-indigo-700">GitHub</span>
                            </button>
                             <button className="flex items-center justify-center gap-2 p-2 border border-slate-300 rounded-lg hover:bg-slate-50">
                                <span>🦊</span>
                                <span className="font-semibold text-slate-600">GitLab</span>
                            </button>
                        </div>
                    </div>
                     <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">支持 GitHub、GitLab 等主流平台</p>
                    </div>
                     <div>
